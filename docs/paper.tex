...
In section 2, we talked about the background and history of Internationalized Domain Name (IDN) and the vulnerability behind it.
...
However, this mechanism is based on the search engine for return spelling suggestion and as a result, it does not play well when it is used as sole detection.
...
- Delay between whois to not saturate server, also only in small subset (about 1k domains)
