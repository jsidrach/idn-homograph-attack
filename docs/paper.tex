...
Our approach to analyze its potential impact is based on clustering IDNs with their canonical counterparts in the Alexa Top 1 million web sites ranking.
...
Firefox (version 54) uses a mixed script detection algorithm based on the ``Moderately Restrictive'' profile defined in the \textit{Unicode Technical Standard \#39}\endnote{\url{http://www.unicode.org/reports/tr39}}.
...
We then use a list of top web sites and domain registrants data to classify each homograph domain name.
...
For the purposes of this project, only the \texttt{.com} domains have been considered.
...
We use the \textit{.com snapshot} to obtain all the internationalized domains of the \texttt{.com} domain zone.
...
Only the domain name is relevant for this project, so the rest of the columns of the \textit{.com snapshot} can be discarded.
We sort the domains in lexicographical order, removing duplicates in the process.

We use the \textit{top domains snapshot} as a source for the canonical (non-international) domain name of a website, based on its ranking.
...
To detect potential internationalized domain name homograph attacks, we cluster the IDNs in the \textit{.com snapshot}.
...
We group the homograph domain names by its registrant organization, and rank each organization by the number of homograph domain names to their name.
...
This classification is done manually, to be able to differentiate between scam and unrelated websites.
...
From the \textit{.com snapshot}, we identify a total of 1044301 internationalized domains.
